        ...
        for name in self.custom_names:
            ...
            if fuzzy_match:
                if fuzz.ratio(text_to_check, name_to_check) >= threshold:
                    ...
                    redacted_text = re.sub(rf'{re.escape(name)}', replacement, redacted_text, flags=flags)}', replacement, redacted_text, flags=flags)}', replacement, redacted_text, flags=flags)
        else:
