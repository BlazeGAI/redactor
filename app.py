...
try:
    import spacy
    # Load the English model; install via:
    #   python -m spacy download en_core_web_sm
    # See https://spacy.io/models/en#en_core_web_sm
...
class DocumentRedactor:
        ...
        for n in range(min_words, max_words + 1):
            for i in range(len(tokens) - n + 1):
                ...
                if all(re.match(r"^[A-Z][a-zA-Z'’-]+$", w) for w in words):
                    if not any(
                        w.lower() in (org.lower() for org in org_exclusions)
                        for w in words
                    ):
                        seqs.add(seq)
        return seqs
    ...
    def detect_human_names(self, text):
        """
        Combine spaCy NER, honorific regex,
        and TitleCase n-grams for name detection.
        """
        names = set()
...
def main():
    ...
    uploaded = st.file_uploader(
        "Choose a file (DOCX, PPTX, PDF)", type=["docx", "ppt", "pptx", "pdf"]
    )
    if uploaded:
        if not redactor.custom_names:
            st.warning(
                "No name list—scanning first two pages/slides for human names."
            )
